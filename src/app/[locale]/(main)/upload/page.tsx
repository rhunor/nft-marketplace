...
export default function UploadPage() {
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALL_ACCEPTED_TYPES.includes(selectedFile.type)) {
      setNotification({ type: 'error', title: t('notifications.invalidFileType.title'), message: t('notifications.invalidFileType.message') });
      return;
    }

    if (selectedFile.size > 100 * 1024 * 1024) {
      setNotification({ type: 'error', title: t('notifications.fileTooLarge.title'), message: t('notifications.fileTooLarge.message') });
      return;
    }
  ...
  const addCollectionItem = () => {
    if (collectionItems.length >= 20) {
      setNotification({ type: 'warning', title: t('notifications.maxItemsReached.title'), message: t('notifications.maxItemsReached.message') });
      return;
    }
  ...
  const removeCollectionItem = (id: string) => {
    if (collectionItems.length <= 2) {
      setNotification({ type: 'warning', title: t('notifications.minItemsRequired.title'), message: t('notifications.minItemsRequired.message') });
      return;
    }
  ...
  const handleCollectionFileChange = (id: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALL_ACCEPTED_TYPES.includes(selectedFile.type)) {
      setNotification({ type: 'error', title: t('notifications.collectionInvalidFileType.title') });
      return;
    }

    if (selectedFile.size > 100 * 1024 * 1024) {
      setNotification({ type: 'error', title: t('notifications.collectionFileTooLarge.title'), message: t('notifications.collectionFileTooLarge.message') });
      return;
    }
  ...
  const handleSingleSubmit = async (e: React.FormEvent) => {
    ...
    if (!file) {
      setNotification({ type: 'error', title: t('notifications.noFileSelected.title') });
      return;
    }

    if (hasInsufficientBalance) {
      setNotification({ type: 'warning', title: t('notifications.insufficientBalance.title'), message: t('notifications.insufficientBalance.uploadMessage', { fee: formatETH(currentFee) }) });
      return;
    }

    if (!formData.price || formData.price <= 0) {
      setErrors((prev) => ({ ...prev, price: t('form.single.priceError') }));
      return;
    }
    ...
    try {
      ...
      setNotification({ type: 'success', title: t('notifications.nftCreated.title'), message: t('notifications.nftCreated.message') });
      setTimeout(() => router.push(`/nft/${nftData?.data?.nft?._id}`), 1500);
    } catch (error) {
      setNotification({
        type: 'error',
        title: t('notifications.uploadFailed.title'),
        message: error instanceof Error ? error.message : t('notifications.uploadFailed.defaultMessage'),
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCollectionSubmit = async (e: React.FormEvent) => {
    ...
    if (!collectionName.trim()) newErrors.collectionName = t('form.collection.requiredError');
    if (!collectionDescription.trim()) newErrors.collectionDescription = t('form.collection.requiredError');

    // Validate collection price
    const price = parseFloat(collectionPrice);
    if (isNaN(price) || price <= 0) {
      newErrors.collectionPrice = t('form.collection.priceError');
    }

    let hasItemErrors = false;
    collectionItems.forEach((item) => {
      if (!item.file) { newErrors[`item_${item.id}_file`] = t('form.collection.requiredError'); hasItemErrors = true; }
      if (!item.title.trim()) { newErrors[`item_${item.id}_title`] = t('form.collection.requiredError'); hasItemErrors = true; }
      // Description is now optional for collection items
    });

    if (Object.keys(newErrors).length > 0) {
      setCollectionErrors(newErrors);
      if (hasItemErrors) setNotification({ type: 'error', title: t('notifications.fillRequiredFields.title') });
      return;
    }

    if (hasInsufficientBalance) {
      setNotification({ type: 'warning', title: t('notifications.insufficientBalance.title'), message: t('notifications.insufficientBalance.collectionMessage', { fee: formatETH(currentFee) }) });
      return;
    }
    ...
    try {
      ...
      setNotification({
        type: 'success',
        title: t('notifications.collectionCreated.title'),
        message: t('notifications.collectionCreated.message', { count: colData?.data?.uploadedCount ?? collectionItems.length }),
      });
    ...
    <div className="py-6 sm:py-8">
      <div className="section-container max-w-4xl">
        ...
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl font-bold sm:text-4xl">{t('header.title')}</h1>
          <p className="mt-2 text-sm sm:text-base text-foreground-muted">{t('header.subtitle')}</p>
        </div>
        ...
        <div className="mb-6 sm:mb-8 grid gap-3 sm:gap-4 grid-cols-2">
          ...
          >
            ...
            <div className="text-center sm:text-left">
              <h3 className="text-sm sm:text-base font-semibold">{t('mode.single.title')}</h3>
              <p className="text-xs sm:text-sm text-foreground-muted hidden sm:block">{t('mode.single.subtitle')}</p>
            </div>
          ...
          >
            ...
            <div className="text-center sm:text-left">
              <h3 className="text-sm sm:text-base font-semibold">{t('mode.collection.title')}</h3>
              <p className="text-xs sm:text-sm text-foreground-muted hidden sm:block">{t('mode.collection.subtitle')}</p>
            </div>
        ...
        <Card className="mb-6 sm:mb-8 p-3 sm:p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs sm:text-sm text-foreground-muted">{t('fee.label')}</p>
              ...
              <p className="text-xs text-foreground-subtle">
                {mode === 'single'
                  ? t('fee.singleDescription')
                  : t('fee.collectionDescription', { count: collectionItems.length, total: (200 * collectionItems.length).toLocaleString() })
                }
              </p>
            </div>
            <div className="sm:text-right">
              <p className="text-xs sm:text-sm text-foreground-muted">{t('fee.yourBalance')}</p>
          ...
          {hasInsufficientBalance && (
            <div className="mt-3 flex items-center gap-2 rounded-lg bg-error/10 p-2 sm:p-3 text-xs sm:text-sm text-error">
              ...
              <span>{t('fee.insufficientBalance')}</span>
            </div>
          )}
        </Card>
          ...
          <form onSubmit={handleSingleSubmit}>
            <Card className="p-4 sm:p-6">
              <div className="space-y-4 sm:space-y-6">
                {/* File Upload */}
                <div>
                  <label className="mb-2 block text-sm font-medium">{t('form.single.mediaLabel')}</label>
                  {!file ? (
                    <label
                      onDrop={handleDrop}
                    ...
                    >
                      ...
                      <p className="mb-1 sm:mb-2 text-center text-sm sm:text-base font-medium">{t('form.single.dragDrop')}</p>
                      <p className="text-center text-xs sm:text-sm text-foreground-muted">{t('form.single.acceptedTypes')}</p>
                ...
                <Input label={t('form.single.titleLabel')} name="title" value={formData.title} onChange={handleInputChange} error={errors.title} placeholder={t('form.single.titlePlaceholder')} maxLength={100} />

                <div className="grid gap-4 sm:grid-cols-2">
                  <Input label={t('form.single.priceLabel')} name="price" type="number" step="any" min="0" value={priceInput} onChange={handleInputChange} onBlur={handlePriceBlur} error={errors.price} placeholder={t('form.single.pricePlaceholder')} />
                  <Select label={t('form.single.categoryLabel')} name="category" value={formData.category} onChange={handleInputChange} options={categoryOptions} />
                </div>

                <div>
                  <label className="mb-2 block text-sm font-medium">{t('form.single.tagsLabel')}</label>
                  <div className="flex gap-2">
                    <Input value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder={t('form.single.tagsPlaceholder')} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag(); } }} />
                    <Button type="button" variant="secondary" onClick={addTag}>{t('form.single.addTag')}</Button>
                  </div>
                ...
                <Button type="submit" className="w-full" disabled={isLoading || hasInsufficientBalance} leftIcon={isLoading ? undefined : <Upload className="h-5 w-5" />}>
                  {isLoading ? t('form.single.submitting') : t('form.single.submit')}
                </Button>
              </div>
            </Card>
          </form>
        ...
        {mode === 'collection' && (
          <form onSubmit={handleCollectionSubmit}>
            <Card className="mb-4 sm:mb-6 p-4 sm:p-6">
              <h2 className="mb-4 text-base sm:text-lg font-semibold">{t('form.collection.detailsHeading')}</h2>
              <div className="space-y-4">
                <Input label={t('form.collection.nameLabel')} value={collectionName} onChange={(e) => { setCollectionName(e.target.value); setCollectionErrors((p) => ({ ...p, collectionName: '' })); }} error={collectionErrors.collectionName} placeholder={t('form.collection.namePlaceholder')} maxLength={100} />
                <Textarea label={t('form.collection.descriptionLabel')} value={collectionDescription} onChange={(e) => { setCollectionDescription(e.target.value); setCollectionErrors((p) => ({ ...p, collectionDescription: '' })); }} error={collectionErrors.collectionDescription} placeholder={t('form.collection.descriptionPlaceholder')} maxLength={2000} />
                <div className="grid gap-4 sm:grid-cols-2">
                  <Select label={t('form.collection.categoryLabel')} value={collectionCategory} onChange={(e) => setCollectionCategory(e.target.value as NFTCategory)} options={categoryOptions} />
                  <Input
                    label={t('form.collection.priceLabel')}
                    type="number"
                    step="any"
                    min="0.01"
                    ...
                    placeholder={t('form.collection.pricePlaceholder')}
                    hint={t('form.collection.pricePerItemHint', { price: formatETH(parseFloat(collectionPrice || '0') / Math.max(collectionItems.length, 1)) })}
                  />
                </div>
              </div>
            </Card>
            ...
            <div className="mb-3 sm:mb-4 flex items-center justify-between">
              <h2 className="text-base sm:text-lg font-semibold">{t('form.collection.itemsHeading', { count: collectionItems.length })}</h2>
              ...
              {collectionItems.map((item, index) => (
                <Card key={item.id} className="p-3 sm:p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <h3 className="text-sm font-medium">{t('form.collection.itemLabel', { number: index + 1 })}</h3>
                  ...
                  {!item.file ? (
                    <label className="mb-3 flex cursor-pointer items-center justify-center rounded-xl border-2 border-dashed border-border bg-background-hover p-4 transition-colors hover:border-accent-primary">
                      <div className="text-center">
                        ...
                        <p className="text-xs sm:text-sm">{t('form.collection.itemUploadPrompt')}</p>
                      </div>
                  ...
                  <div className="space-y-3">
                    <Input label={t('form.collection.itemTitleLabel')} value={item.title} onChange={(e) => updateCollectionItem(item.id, 'title', e.target.value)} error={collectionErrors[`item_${item.id}_title`]} placeholder={t('form.collection.itemTitlePlaceholder')} maxLength={100} />
                  </div>
                </Card>
              ))}
            </div>
            ...
            <Button type="submit" className="mt-6 w-full" disabled={isLoading || hasInsufficientBalance} leftIcon={isLoading ? undefined : <Upload className="h-5 w-5" />}>
              {isLoading ? t('form.collection.submitting') : t('form.collection.submit', { count: collectionItems.length })}
            </Button>
          </form>
        )}
      </div>

...
function ExploreContent() {
  ...
  const sortOptions = useMemo(
    () => [
      ...
      { value: 'recent-desc', label: t('sort.recentDesc') },
      { value: 'volume-desc', label: t('sort.volumeDesc') },
      { value: 'volume-asc', label: t('sort.volumeAsc') },
      { value: 'floor-asc', label: t('sort.floorAsc') },
      { value: 'floor-desc', label: t('sort.floorDesc') },
      { value: 'items-desc', label: t('sort.itemsDesc') },
    ],
    [t]
  );
        ...
        {dbNFTs.length > 0 && (
          <section className="mb-12">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold">{t('newUploads.heading')}</h2>
                <p className="text-sm text-foreground-muted">
                  {t('newUploads.subtitle')}
                </p>
              </div>
            </div>
            ...
            {dbNFTs.length > 4 && (
              <div className="mt-4 text-center">
                ...
                >
                  {t('newUploads.viewAll', { count: dbNFTs.length })}
                </Button>
              </div>
            )}
          </section>
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder={viewMode === 'collections' ? t('search.collectionsPlaceholder') : t('search.nftsPlaceholder')}
          ...
          <div className="flex-1">
            ...
            {hasActiveFilters && (
              <div className="mb-6 flex flex-wrap items-center gap-2">
                <span className="text-sm text-foreground-muted">{t('activeFilters')}</span>
                {searchQuery && (
            ...
            <p id="results-section" className="mb-6 text-sm text-foreground-muted">
              {viewMode === 'collections'
                ? t('resultsCount.collections', { count: filteredCollections.length })
                : t('resultsCount.items', { count: filteredNFTs.length + dbNFTs.length })
              }
            </p>
                          ...
                          <div className="relative aspect-[4/3] overflow-hidden">
                            ...
                            <div className="absolute bottom-4 left-4 right-4">
                              ...
                              <p className="mt-1 text-sm text-white/80">
                                {t('itemsCount', { count: collection.totalItems })}
                              </p>
                            </div>
                          </div>
                          <div className="p-4">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                <p className="text-xs text-foreground-muted">{t('floor')}</p>
                                <p className="font-bold text-accent-primary">
                            ...
                            <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
                              <div>
                                <p className="text-xs text-foreground-muted">{t('totalVolume')}</p>
                                <p className="font-semibold">{formatETH(collection.totalVolume)}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-xs text-foreground-muted">{t('usd')}</p>

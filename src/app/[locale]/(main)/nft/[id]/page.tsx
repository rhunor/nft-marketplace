...
export default function NFTDetailPage() {
  const t = useTranslations('nftDetail.detail');
  ...
  const fetchNFT = useCallback(async () => {
    try {
      ...
      if (isDbNft) {
        // Fetch from database
        const response = await fetch(`/api/nfts/${nftId}?locale=${locale}`);
        ...
        if (foundNft) {
          ...
          throw new Error(t('errors.notFoundError'));
        }
      }
    } catch (err) {
  ...
  }, [nftId, isDbNft, session?.user?.id, t, locale]);
    ...
    if (isDbNft) {
      ...
      try {
        ...
        if (result.ok) {
          ...
          setNotification({
            type: 'success',
            title: data.data.liked ? t('notifications.addedToFavorites') : t('notifications.removedFromFavorites'),
          });
        }
      } catch {
        setNotification({
          type: 'error',
          title: t('notifications.likeUpdateFailed'),
        });
      }
    } else {
      ...
      setNotification({
        type: 'success',
        title: isLiked ? t('notifications.removedFromFavorites') : t('notifications.addedToFavorites'),
      });
    }
  };
  ...
  const confirmPurchase = async () => {
    if (isDbNft && dbNft) {
      ...
      try {
        ...
        setNotification({
          type: 'success',
          title: t('notifications.purchaseSuccessTitle'),
          message: t('notifications.purchaseSuccessMessage'),
        });
        ...
        setNotification({
          type: 'error',
          title: t('notifications.purchaseFailedTitle'),
          message: err instanceof Error ? err.message : t('notifications.genericError'),
        });
      } finally {
      ...
      setNotification({
        type: 'success',
        title: t('notifications.purchaseSubmittedTitle'),
        message: t('notifications.purchaseSubmittedMessage'),
      });
    }
  };
  ...
  const handleListNFT = async () => {
    ...
    try {
      ...
      if (isNaN(price) || price <= 0) {
        setNotification({ type: 'error', title: t('notifications.invalidPrice') });
        setIsUpdating(false);
        return;
      }
      ...
      setNotification({ type: 'success', title: t('notifications.listedTitle'), message: t('notifications.listedMessage') });
      await fetchNFT();
    } catch (err) {
      setNotification({ type: 'error', title: t('notifications.listFailedTitle'), message: err instanceof Error ? err.message : t('notifications.genericError') });
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const handleUnlistNFT = async () => {
    ...
    try {
      ...
      if (!result.ok) throw new Error(result.error);
      setShowManageModal(false);
      setNotification({ type: 'success', title: t('notifications.unlistedTitle'), message: t('notifications.unlistedMessage') });
      await fetchNFT();
    } catch (err) {
      setNotification({ type: 'error', title: t('notifications.unlistFailedTitle'), message: err instanceof Error ? err.message : t('notifications.genericError') });
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const handleUpdatePrice = async () => {
    ...
    try {
      ...
      if (!result.ok) throw new Error(result.error);
      setShowManageModal(false);
      setNotification({ type: 'success', title: t('notifications.priceUpdatedTitle'), message: t('notifications.priceUpdatedMessage', { price }) });
      await fetchNFT();
    } catch (err) {
      setNotification({ type: 'error', title: t('notifications.priceUpdateFailedTitle'), message: err instanceof Error ? err.message : t('notifications.genericError') });
    } finally {
      setIsUpdating(false);
    }
  };
    ...
    return (
      <div className="section-container py-20 text-center">
        ...
        <p className="mt-2 text-foreground-muted">
          {error || t('notFound.genericMessage')}
        </p>
        <Link href="/explore" className="mt-6 inline-block">
          <Button>{t('notFound.browseButton')}</Button>
        </Link>
      </div>
    );
  }
  ...
  const nft = isDbNft && dbNft
    ? {
        id: dbNft._id,
        title: showOriginal && dbNft.isTranslated ? dbNft.originalTitle ?? dbNft.title : dbNft.title,
        description: showOriginal && dbNft.isTranslated ? dbNft.originalDescription ?? dbNft.description : dbNft.description,
        isTranslated: !!dbNft.isTranslated,
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              {!nft.isListed && (
                <Badge variant="default">{t('badges.notForSale')}</Badge>
              )}
              {isOwner && (
                <Badge variant="success">{t('badges.youOwnThis')}</Badge>
              )}
              {nft.isFromDatabase && (
                <Badge variant="primary">{t('badges.marketplaceNft')}</Badge>
              )}
            </div>
            ...
            <div>
              ...
              {nft.isTranslated && (
                <button
                  ...
                  className="mt-1 flex items-center gap-1 text-xs text-foreground-subtle hover:text-foreground-muted"
                >
                  {t('translated.badge')} ·{' '}
                  <span className="underline">
                    {showOriginal ? t('translated.showTranslation') : t('translated.showOriginal')}
                  </span>
                </button>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-6">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-sm text-foreground-muted">{t('creator')}</p>
                  <p className="font-medium">
                    @{nft.creatorUsername}
                    {isCreator && <span className="ml-1 text-accent-primary">{t('badges.you')}</span>}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-sm text-foreground-muted">{t('owner')}</p>
                  <p className="font-medium">
                    @{nft.ownerUsername}
                    {isOwner && <span className="ml-1 text-accent-primary">{t('badges.you')}</span>}
                  </p>
                </div>
              </div>
            </div>
            ...
            <Card className="p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-sm text-foreground-muted">{t('price.current')}</p>
            ...
            {nft.tags && nft.tags.length > 0 && (
              <div>
                <h2 className="mb-3 text-lg font-semibold">{t('tags')}</h2>
                <div className="flex flex-wrap gap-2">
                  {nft.tags.map((tag) => (
                    <Link
                      key={tag}
            ...
            {/* Details */}
            <div>
              <h2 className="mb-3 text-lg font-semibold">{t('details.heading')}</h2>
              <div className="space-y-3 rounded-xl border border-border bg-background-secondary p-4">
                <div className="flex items-center justify-between">
                  <span className="text-foreground-muted">{t('details.tokenId')}</span>
                  <span className="font-mono text-sm">{nft.id}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-foreground-muted">{t('details.blockchain')}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-foreground-muted">{t('details.mediaType')}</span>
                  <span className="capitalize">{nft.mediaType}</span>
                </div>
                {nft.createdAt && (
                  <div className="flex items-center justify-between">
                    <span className="text-foreground-muted">{t('details.created')}</span>
        ...
        {relatedNFTs.length > 0 && (
          <section className="mt-16">
            <NFTCarousel
              ...
              subtitle={t('related.subtitle', { category: getCategoryLabel(nft.category) })}
            />
          </section>
        )}
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 rounded-xl border border-border p-4">
            ...
            <div>
              ...
              <p className="text-sm text-foreground-muted">
                {t('buyModal.by', { username: nft.creatorUsername })}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2 rounded-xl bg-background-hover p-4">
            <div className="flex justify-between">
              <span className="text-foreground-muted">{t('buyModal.price')}</span>
              <span className="font-medium">{formatETH(nft.price)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-foreground-muted">
                {nft.isFromDatabase ? t('buyModal.platformFee') : t('buyModal.gasFee')}
              </span>
              <span className="font-medium">{formatETH(platformFee)}</span>
            </div>
            <div className="border-t border-border pt-2">
              <div className="flex justify-between">
                <span className="font-medium">{t('buyModal.total')}</span>
          ...
          <div className="rounded-xl bg-background-secondary p-4">
            <div className="flex justify-between text-sm">
              <span className="text-foreground-muted">{t('buyModal.yourBalance')}</span>
            ...
            {hasInsufficientBalance && (
              <p className="mt-2 flex items-center gap-1 text-sm text-error">
                ...
                {t('buyModal.insufficientBalanceWarning')}
              </p>
            )}
          </div>
          ...
          <p className="text-sm text-foreground-muted">
            ...
            {nft.isFromDatabase
              ? t('buyModal.immediateTransferNote')
              : t('buyModal.adminTransferNote')}
          </p>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4 rounded-xl border border-border p-4">
            ...
            <div>
              <h3 className="font-semibold">{nft.title}</h3>
              <p className="text-sm text-foreground-muted">
                {nft.isListed ? t('manageModal.statusListed') : t('manageModal.statusNotListed')}
              </p>
            </div>
          </div>
          ...
          {/* Price Input */}
          <Input
            label={t('manageModal.priceLabel')}
            type="number"
            step="any"
            min="0.001"
            value={managePrice}
            onChange={(e) => setManagePrice(e.target.value)}
            placeholder={t('manageModal.pricePlaceholder')}
